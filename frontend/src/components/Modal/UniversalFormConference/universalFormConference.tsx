import {
  ...
  const UniversalFormConference = ({
    ...
    const { createFast, createScheduled, createRepetitive } = useConferences();
    ...
    const [conferenceData, setConferenceData] =
      useState<FormData>(defaultEmptyData);
  
    function updateFields(fields: Partial<FormData>) {
      setConferenceData((prev) => ({ ...prev, ...fields }));
    }
        ...
        switch (stepIndex) {
          case 0:
            if (!data.title || data.title.trim().length < 5) {
              error = "Название конференции должно быть больше 5 символов";
            }
            break;
          case 1:
            if (!data.started_at || new Date(data.started_at).getTime() < now.getTime()) {
              error = "Дата начала конференции не может быть в прошлом";
            }
            break;
          default:
            break;
    ...
    async function handleConfirm(e: FormEvent) {
      e.preventDefault();
      if (!validateStep(conferenceData, currentIndex)) return;
      ...
      <AlertDialog open={open} onOpenChange={setOpen}>
        {button && (
          <AlertDialogTrigger asChild>
            {button || <button onClick={setOpen}>Открыть</button>}
          </AlertDialogTrigger>
        )}
          ...
          <form onSubmit={handleConfirm}>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2">{step}</div>
              </AlertDialogDescription>
            </AlertDialogHeader>
